...
| -------------- | ------------------------- | ---------------------- |
| 앞에 추가/삭제       | O(n) – 인덱스 O(1) + 이동 O(n) | O(1) – 포인터로 직접 접근 가능   |
| 중간 추가/삭제       | O(n) – 이동 필요              | O(n) – 탐색 후 링크 조정      |
| 뒤에 추가/삭제       | O(1) – 가장 빠름              | O(1) – last 포인터로 바로 추가 |
| 인덱스 조회 (`get`) | O(1) – 배열처럼 바로 접근         | O(n) – 앞에서부터 순차 탐색 필요  |
| 검색 (`indexOf`) | O(n) – 순회하며 검색            | O(n) – 순회하며 검색, 더 느림   |

<br>

> ArrayList는 메모리상에 요소들이 연속적으로 저장되기 때문에 CPU 캐시 효율이 높고, 객체 접근 속도도 빨라 대부분의 상황에서 더 우수한 성능을 보입니다. 반면, LinkedList는 앞부분에서 자주 삽입하거나 삭제하는 경우 포인터만 조작하면 되므로 더 효율적입니
