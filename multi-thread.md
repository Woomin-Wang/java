...
작업 `A → B` 순으로 실행하려면, A 작업의 결과를 `future.get`으로 꺼낸 후 B 작업을 **명시적으로 실행**해야 한다.

```java
Future<String> futureA = executor.submit(() -> "A 결과");
String resultA = futureA.get();  // 블로킹
Future<String> futureB = executor.submit(() -> resultA + " 처리");
```

이때 `get()`은 **블로킹 호출**로 작동하여 스레드가 멈춰 결과를 기다리게 되므로, 비동기 작업임에도 불구하고 동기 방식처럼 동작하게 된다.

<br>
  
**병렬 작업 조합 복잡**
- 여러 Future 작업을 병렬로 실행하고 결과를 종합하려면 각각 `get()`을 수동으로 호출해야 한다.
- 이로 인해 **작업 완료 순서**와 관계없이 일일이 관리해야 하므로 코드가 지저분해지고 비효율적이다. 

**콜백 없음** 
- 작업 완료 후 **자동 실행되는 메커니즘이 없어** `isDone()` 또는 `get()`으로 직접 확인해야 한다.
- **자동 흐름 제어 불가**, 비동기 로직을 수동으로 관리해야 하므로 복잡하다.
...
**2. 예외 처리 불편함**
- **지연된 예외 감지:** 비동기 작업에서 예외가 발생해도, `get()` 호출 시점에 ExecutionException으로 래핑되서 던져짐
- **복구 로직 미지원:**
- **예외 체이닝 어려움:**

  
**3. 후속 작업 연결 어려움**
- **get() 의존:** 결과 확인을 위해 반드시 `get()`을 호출해야 해서 비효율적
- **자동 흐름 없음:**
-
...
Future의 한계를 개선하고 복잡한 비동기 작업 흐름을 간편하게 처리하기 위해, **Java 8에서는 CompletableFuture**가 새롭게 추가되었다.
