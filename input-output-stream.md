...
> FileWriter와 FileReader는 OutputStreamWriter와 InputStreamReader가 제공하는 **인코딩/디코딩 변환 기능을 기반**으로, 파일에 문자 데이터를 쉽고 간결하게 입출력하는데 특화된 문자 스트림이다.
...
### DataInputStream, DataOutputStream (기본 타입 데이터 스트림)

DataInputStream과 DataInputStream은 **자바의 기본 자료형(원시 타입) 데이터를 스트림을 통해 주고 받을 때** 사용되는 보조 스트림이다.

<br>

> 💡 왜 사용할까?
>
> 이들을 사용하는 이유는 **데이터 형식(타입)을 그대로 유지하며 읽고 쓰는 것**에 있다.
>
> 일반적인 바이트 스트림이나 문자 스트림은 데이털르 바이트 배열이나 문자열 형태로만 다룬다.
>
> 예를 들어, 정수 `123`을 파일에 쓸 때, FileWriter를 사용하면 `1`, `2`, `3` 이라는 문자가 각자의 인코딩에 따라 바이트로 변환되어 저장된다.
>
> 다시 읽을 때도 `1`, `2`, `3`이라는 문자을 읽어와 개발자가 직접 정수 `123`으로 변환해야 한다.

<br>

하지만 DataOutputStream과 DataOutputStream은 다르다:

- **타입 보존:** DataOutputStream은 `writeInt(123) 메서드를 사용하면, 정수 `123`이 정수로서의 4바이트 바이너리 값으로 변환되어 스트림에 기록된다.

- **정확한 복원:** DataInputStream은 `readInt()` 메서드로 읽으면, 해당 4바이트를 읽어와 정확히 정수 `123`으로 디코딩 해준다.

<br>

이러한 특성 때문에 다음과 같은 상황에서 유용하다:

- **바이너리 파일 처리:** 텍스트 파일이 아닌 이미지, 오디오, 구조화된 바이너리 데이터 등을 파일에 저장하고 읽을 때, 데이터의 원본 형태를 그대로 보존해야 할 경우 사용

- **네트워크 통신:** 클라이언트와 서버 간에 숫자나 불린 값 같은 특정 형식의 데이터를 주고받을 때, 데이터의 손실이나 오해석 없이 통신해야할 때 필수적

- **데이터 무결성:** 읽고 쓰는 과정에서 데이터 타입이 변경되지 않고 그 값이 유지되어야 하는 상황에서 데이터의 무결성을 보장하는 데 도움

<br>

> 문자 스트림은 문자 데이터를 개발자가 쉽게 다룰 수 있도록 내부적으로 문자 인코딩/디코딩을 처리한다.
>
> **Data 스트림도 같은 맥락에서 int, double 같은 원시 타입 데이터를 자동으로 바이트 변환 해주는 스트림이다.**
