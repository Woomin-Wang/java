...
`DataInputStream`과 `DataInputStream`은 **자바의 기본 자료형(원시 타입) 데이터를 스트림을 통해 주고 받을 때** 사용되는 보조 스트림이다.
...
>
> 일반적인 바이트 스트림이나 문자 스트림은 데이터를 바이트 배열이나 문자열 형태로만 다룬다.
>
...
- **타입 보존:** `DataOutputStream`은 `writeInt(123) 메서드를 사용하면, 정수 `123`이 정수로서의 4바이트 바이너리 값으로 변환되어 스트림에 기록된다.

- **정확한 복원:** `DataInputStream`은 `readInt()` 메서드로 읽으면, 해당 4바이트를 읽어와 정확히 정수 `123`으로 디코딩 해준다.
